...
public class StoryDetail {
    ...
    public View createView(final ActivityHelper helper) {
        ...
        if (story.has(Story.StoryKey.FAVORITE_STATE)) {
            ...
            root.findViewById(R.id.favorite).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    helper.executeTask(new Runnable() {
                        ...
                        public void run() {
                            try {
                                FavoriteState n = story.<FavoriteState>get(Story.StoryKey.FAVORITE_STATE)
                                                       .isFavorited() ?
            ...
            root.findViewById(R.id.favorite).setVisibility(View.GONE);
        }
        if (story.has(Story.StoryKey.READ_LATER_STATE)) {
            ...
            root.findViewById(R.id.readlater).setVisibility(View.GONE);
        }
        if (getImage() != null) {
            helper.executeTask(new Runnable() {
                @Override
        ...
        for (final Chapter chapter : story.<List<Chapter>>get(Story.StoryKey.CHAPTERS)) {
            ...
            chapterView.findViewById(R.id.unread_query)
                       .setVisibility(chapter.getBoolean(Chapter.ChapterKey.UNREAD, false) ? View.VISIBLE : View.GONE);
            if (chapter.has(Chapter.ChapterKey.UNREAD)) {
                chapterView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        helper.executeTask(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    chapter.set(AccountActions.toggleRead(helper.getSession().getHttpClient(),
                                                                          chapter));
                                    ...
                                    log.error("Could not toggle read status for " +
                                              chapter.getInt(Chapter.ChapterKey.ID), e);
                                }
                            }
                        });
                    }
